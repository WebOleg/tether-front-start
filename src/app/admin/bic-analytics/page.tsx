...
import type { BicAnalyticsStats, BicAnalyticsDetail } from '@/types'
...
export default function BicAnalyticsPage() {
  ...
  const filteredBics = useMemo(() => {
    ...
    return result
  }, [bicStats, countryFilter, bicSearch, highRiskOnly, sortField])
  ...
  const hasActiveFilters = countryFilter !== 'all' || bicSearch !== '' || highRiskOnly
          ...
          <CardContent>
            {loading ? (
              ...
              <p className="text-slate-500 text-center py-4">
                {hasActiveFilters ? 'No BICs match the filters' : 'No BIC data available'}
              </p>
            )}
          </CardContent>
        </Card>
